...
- All `EditText` fields converted to Compose `OutlinedTextField` across the app:
  ...
  - `dialog_user_agent.xml`, `item_custom_list.xml`, `bottom_sheet_file_explorer.xml`
...
- AutoCompleteTextView dropdown selectors (search bar, media list filters) are NOT converted — they are selection widgets, not text input fields.
